...
import type { DetailedHTMLProps, HTMLProps, OlHTMLAttributes } from "react";
...
export const ExternalLinkPopup = ({
	url,
	onClose,
	onNavigate,
	...
	const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
		...
		const isHeader =
			target.closest("h3") !== null || target.classList.contains("drag-handle");
		...
		>
			<Card className="border-0 shadow-none gap-2">
				...
				<CardFooter className="flex justify-end gap-2 p-2 pt-0">
					...
					<Button onClick={onNavigate} className="h-9">
						Go to Link
					</Button>
				</CardFooter>
			</Card>
