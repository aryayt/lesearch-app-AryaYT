	...
	try {
		...
		if (!token || !token.idToken) {
			...
			return NextResponse.redirect(
				new URL("/login?error=no-id-token", baseUrl),
			);
		}

		// Create a response object for redirect
		const response = NextResponse.redirect(new URL("/documents", baseUrl));
			...
			if (error.message.includes("User from sub claim in JWT does not exist")) {
				...
				try {
					...
					if (signUpError) {
						...
						return NextResponse.redirect(
							new URL(
								`/login?error=${encodeURIComponent(signUpError.message)}`,
								baseUrl,
							),
						);
					}
