	...
	const verifyOtpFromUrl = async (emailParam: string, otpParam: string) => {
		try {
			...
			setVerificationError(
				(err as Error).message || "Invalid or expired verification code",
			);
	...
	const renderForm = () => {
				...
				<form onSubmit={handleResetPassword} className="space-y-6">
					<div className="space-y-2">
						...
						<div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-1 text-sm">
							<div
								className={`flex items-center space-x-2 ${/[A-Z]/.test(password) ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}
							>
								{/[A-Z]/.test(password) ? (
									<Check size={14} />
								) : (
							...
							<div
								className={`flex items-center space-x-2 ${/[0-9]/.test(password) ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}
							>
								{/[0-9]/.test(password) ? (
									<Check size={14} />
								) : (
							...
							<div
								className={`flex items-center space-x-2 ${/[^A-Za-z0-9]/.test(password) ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}
							>
							...
							<div
								className={`flex items-center space-x-2 ${password.length >= 8 ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}`}
							>
								{password.length >= 8 ? (
									<Check size={14} />
								) : (
					...
					<div className="space-y-2">
						...
						{password && confirmPassword && (
							<div
								className={`flex items-center space-x-2 mt-1 text-sm ${password === confirmPassword ? "text-green-600 dark:text-green-400" : "text-red-500"}`}
							>
								{password === confirmPassword ? (
									<Check size={14} />
								) : (
					...
					<Button type="submit" className="w-full" disabled={loading}>
						{loading ? (
							<span className="animate-pulse text-sm">Please wait...</span>
						) : (
							"Reset Password"
						)}
					</Button>
		...
		return (
			<form onSubmit={handleSendOtp} className="space-y-6">
				...
				<Button type="submit" className="w-full" disabled={loading}>
					{loading ? (
						<span className="animate-pulse text-sm">Sending otp...</span>
					) : (
						"Send Reset Code"
					)}
				</Button>
					...
					<div className="p-6">
						<div className="flex flex-col items-center">
							...
							<h1 className="mb-1 mt-4 text-xl font-semibold">
								{passwordReset ? "Create New Password" : "Reset Your Password"}
							</h1>
							<p>
								{verificationError ? (
