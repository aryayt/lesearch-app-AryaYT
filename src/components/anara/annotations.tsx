...
export const SelectionTooltipContent = ({
	onHighlight,
	onAskAI,
}: SelectionTooltipContentProps) => {
	return (
		<ToolbarGroup>
			<Button variant="ghost" size="sm" onClick={onAskAI} className="gap-2">
				<svg
					fill="url(#myGradient)"
					viewBox="0 0 512 512"
				...
				<span
					style={{
						background:
							"linear-gradient(to bottom right, #6EB6F2 0%, #6EB6F2 15%, #c084fc 40%, #f87171 60%, #fcd34d 100%)",
...
export const AnnotationTooltip = ({
	...
	const { updateAnnotation, deleteAnnotation } = useAnnotationActions(
