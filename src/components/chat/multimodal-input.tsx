	...
	const validateFile = useCallback(
		(file: File) => {
			...
			if (!isAllowedType && !isMarkdown && !hasValidExtension) {
				toast.error(
					`File type ${file.type} is not supported. Please upload images, documents (PDF, DOC, DOCX, MD, TXT, RTF), spreadsheets (XLS, XLSX), presentations (PPT, PPTX), or data files (CSV, JSON).`,
				);
				return false;
			}
				...
				{selectedText && (
					<div className="flex flex-col w-full rounded-lg border border-muted bg-muted/40 px-4 py-2 mb-3 text-sm text-foreground/90 gap-2 overflow-hidden group">
						<div className="flex items-center gap-3 min-w-0">
							...
							<span className="truncate font-medium" title={selectedText.text}>
								{selectedText.text}
							</span>
