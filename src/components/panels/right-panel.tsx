...
const RightPanel = () => {
	...
	useEffect(() => {
		const handleClickOutside = (event: MouseEvent) => {
			if (
				(event.target as Element).closest('button[aria-label="Chat History"]')
			) {
				return;
			}
			...
			if (
				...
				!historyCardRef.current.contains(event.target as Node)
			) {
				setShowHistory(false);
			}
		};
	...
	return (
		<Card className="flex flex-col h-full w-full gap-1 relative">
			...
			{showHistory && (
				<Card
					ref={historyCardRef}
					className="absolute right-0 top-8 w-80 shadow-lg border rounded-md"
				>
