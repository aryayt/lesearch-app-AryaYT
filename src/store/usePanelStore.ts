...
interface PanelStore {
	...
	removeTab: (tabId: string, panel: "left" | "middle") => void;
	...
	try {
		...
		if (pageType === "pdf") {
			...
			if (matchError) {
				...
				throw new Error(
					`Database error while searching for PDF: ${matchError.message || "Unknown error"}`,
				);
			}
			...
			if (!allMatches || allMatches.length === 0) {
				...
				throw new Error(
					`PDF with ID "${pageId}" not found. Verify the ID is correct and the PDF exists in the database.`,
				);
			}

			if (allMatches.length > 1) {
				console.error(
					`Multiple PDFs (${allMatches.length}) found with the same ID "${pageId}"`,
				);
				throw new Error(
					`Database integrity error: Multiple PDFs found with ID "${pageId}". Please contact the database administrator.`,
				);
			}
		...
		if (matchError) {
			...
			throw new Error(
				`Database error while searching for Note: ${matchError.message || "Unknown error"}`,
			);
		}
		...
		if (!allMatches || allMatches.length === 0) {
			...
			throw new Error(
				`Note with ID "${pageId}" not found. Verify the ID is correct and the note exists in the database.`,
			);
		}

		if (allMatches.length > 1) {
			console.error(
				`Multiple Notes (${allMatches.length}) found with the same ID "${pageId}"`,
			);
			throw new Error(
				`Database integrity error: Multiple Notes found with ID "${pageId}". Please contact the database administrator.`,
			);
		}
		...
		throw new Error(
			`Error fetching ${pageType} with ID "${pageId}": ${String(err)}`,
		);
	}
}
...
type PersistedState = Pick<
	PanelStore,
	"pageTabs" | "activePageId" | "leftActiveTabId" | "panelVisibility"
>;
			...
			addTab: async (pageId, pageType, panel) => {
				...
				try {
					...
					if (!pageType || !["pdf", "note"].includes(pageType)) {
						throw new Error(
							`Invalid pageType: ${pageType}. Must be 'pdf' or 'note'`,
						);
					}
					...
					if (!activePageId) {
						throw new Error(
							"No active page selected. Please select a page before adding tabs or use the left panel to add tabs.",
						);
					}
					...
					const targetTabs =
						panel === "left"
							? existing.leftPanelTabs
							: existing.middlePanelTabs;
					...
					set((state) => {
						...
						return {
							...
							leftActiveTabId:
								panel === "left" ? tab.id : state.leftActiveTabId,
							middleActiveTabId:
								panel === "middle" ? tab.id : state.middleActiveTabId,
						};
					});
			...
			removeTab: (tabId, panel) => {
				...
				set((state) => {
					...
					if (panel === "left") {
						...
						if (
							currentLeftActiveTabId === tabId &&
							updated.leftPanelTabs.length > 0
						) {
							newLeftActiveTabId = updated.leftPanelTabs[0].id;
							// Use setTimeout to ensure state updates properly
							setTimeout(
								() => get().setLeftActiveTabId(newLeftActiveTabId),
								50,
							);
						}
					} else {
						...
						// If the removed tab was active, select another tab
						if (
							currentMiddleActiveTabId === tabId &&
							updated.middlePanelTabs.length > 0
						) {
							newMiddleActiveTabId = updated.middlePanelTabs[0].id;
							// Use setTimeout to ensure state updates properly
							setTimeout(
								() => get().setMiddleActiveTabId(newMiddleActiveTabId),
								50,
							);
						}
					}
				...
				set((state) => {
					...
					for (const pageId of Object.keys(updatedPageTabs)) {
						...
						if (page.leftPanelTabs) {
							page.leftPanelTabs = page.leftPanelTabs.filter(
								(tab) => tab.id !== tabId,
							);
						}
						...
						if (page.middlePanelTabs) {
							page.middlePanelTabs = page.middlePanelTabs.filter(
								(tab) => tab.id !== tabId,
							);
						}
					}
					...
					if (state.leftActiveTabId === tabId) {
						...
						if (
							activePageId &&
							updatedPageTabs[activePageId]?.leftPanelTabs.length > 0
						) {
							newLeftActiveTabId =
								updatedPageTabs[activePageId].leftPanelTabs[0].id;
							setTimeout(
								() => get().setLeftActiveTabId(newLeftActiveTabId),
								50,
							);
					...
					if (state.middleActiveTabId === tabId) {
						...
						if (
							activePageId &&
							updatedPageTabs[activePageId]?.middlePanelTabs.length > 0
						) {
							newMiddleActiveTabId =
								updatedPageTabs[activePageId].middlePanelTabs[0].id;
							setTimeout(
								() => get().setMiddleActiveTabId(newMiddleActiveTabId),
								50,
							);
